...
Launch the entire panel in one `subagent` call using the current scripted-workflow API:

- Put every persona in one `workflowScript` call to `await runs.all([...])`.
- Give every child a stable, descriptive key and `agent: "reviewer"`.
- Pass `artifacts: false` on the top-level tool call. Panel runs must never persist raw
  prompts, inputs, metadata, or transcripts.
- Launch asynchronously, then wait for that exact run with `subagent_wait({ id })` because
  the panel contract requires synthesis from every response before returning to the user.
- Do not use the removed top-level `tasks`, `parallel`, or `chain` fields, and do not launch
  the personas as separate tool calls.

The workflow script should have this shape, with the complete constructed prompts substituted:

```js
const results = await runs.all([
  { key: "operations", agent: "reviewer", task: operationsPrompt },
  { key: "reliability", agent: "reviewer", task: reliabilityPrompt }
]);
return results.map((result) => ({ key: result.key, output: result.output }));
```

If a child fails, report that specific child and its actual startup/runtime error. Never
synthesize a viewpoint for a child that produced no response.
