...
export function driveWorkspaceFallback(query) {
  return [
    ...
    `Search datadog-google-workspace for: ${JSON.stringify(String(query).slice(0, 800))}.`,
...
export function driveIntegrityMessage() {
  return [
    ...
    "For each item, use datadog-google-workspace get_file_metadata and get_file_content with its exact snake_case file_id.",
  ...
  const blocks = items.map((item, index) => {
    ...
    if (item.rehydration?.provider === "google-drive") {
      lines.push(
        ...
        `Before relying on this memory, fetch Drive file ${item.rehydration.driveFileId} with datadog-google-workspace get_file_content, then call journal_rehydrate_drive_memory with that file ID and the exact read-back text.`
      );
