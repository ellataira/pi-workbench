...
`datadog-google-workspace` for at most three candidates. It fetches only the
...
| `datadog-google-workspace` | Search, inspect, and retrieve Drive, Docs, Sheets, and Slides through a lazy OAuth bridge |
...
- For Google Drive, use `/mcp reconnect datadog-google-workspace`. Complete the
...
| Google Drive needs authentication | `/mcp reconnect datadog-google-workspace`; approve in the browser and return to Pi |
...
| Retention candidates remain local | Run `/memory audit`; reconnect `datadog-google-workspace`, then invoke `/memory cleanup`. Missing/mismatched summaries intentionally fail closed |
